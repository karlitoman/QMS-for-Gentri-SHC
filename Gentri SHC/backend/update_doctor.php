...
$stmt = $pdo->prepare('SELECT user_id FROM users WHERE user_id = :id AND role = "doctor"');
$stmt->execute([':id' => $id]);
if (!$stmt->fetch()) {
  json_err(404, 'Doctor not found');
  exit;
}
...
$sql = 'UPDATE users SET ' . implode(', ', $fields) . ' WHERE user_id = :id AND role = "doctor"';
$stmt = $pdo->prepare($sql);
$stmt->execute($params);

json_ok(null, ['message' => 'Doctor updated successfully']);