...
(function setupQueueActions(){
  ...
  async function handleQueueAction(deptName, lane, type) {
    try {
      ...
      alert('Failed to update queue: ' + (err?.message || 'Unknown error'));
    }
  }
  document.addEventListener('click', (e) => {
    ...
    const m = pid.match(/^([a-z-]+)-(priority|regular)-panel$/i);
    if (!m) return;
    const slug = m[1];
    const lane = m[2].toLowerCase();
  ...
  document.addEventListener('click', async (e) => {
    ...
    try {
      ...
      if (nextBtn) {
        ...
        if (!num || isNaN(num)) { alert('Invalid number'); return; }
      ...
      if (!target) { alert('No matching patient found'); return; }
...
function setupMaintenanceUI(){
  ...
  if (diag) diag.addEventListener('click', async ()=>{ try { await postJSON('backend/run_diagnostics.php',{}); showToast('Diagnostics completed','success'); appendLog('System Backup','Success'); } catch(e){ showToast('Diagnostics failed','error'); appendLog('Diagnostics','Issues Found'); } });
  ...
  if (bkp) bkp.addEventListener('click', async ()=>{ try { await postJSON('backend/backup_data.php',{}); showToast('Backup completed','success'); appendLog('System Backup','Success'); } catch(e){ showToast('Backup failed','error'); appendLog('Backup','Failed'); } });
...
function openAdminProfileModal() {
  ...
  if (form) {
    form.onsubmit = (e) => {
      ...
      if (!fn || !ln || !em) { alert('First, Last, and Email are required.'); return; }
      try {
...
async function openEditPatientModal(patientId) {
  try {
    ...
    // Ensure age auto-calculation is wired for edit form fields
    try { setupAgeAutoCalc(); } catch (_) {}
  } catch (error) {
...
if (editPatientForm) {
  editPatientForm.addEventListener('submit', async (e) => {
    ...
    if (!formData.first_name || !formData.last_name || !formData.age || !formData.date_of_birth || !formData.sex) {
      alert('Please fill in all required fields (First Name, Last Name, Age, Date of Birth, Sex).');
      return;
    }
...
if (deptForm) {
  deptForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      alert(err.message || 'Add department failed');
    }
  });
}
...
if (staffForm) {
  staffForm.addEventListener('submit', async (e) => {
    ...
    if (!last || !first || !email) { alert('Last, First, and Email are required'); return; }
    if (pass || pass2) {
    ...
    try {
      ...
      alert(err.message || 'Add staff failed');
    }
  });
}
...
if (doctorForm) {
  doctorForm.addEventListener('submit', async (e) => {
    ...
    if (pass || pass2) { if (pass !== pass2) { alert('Passwords do not match'); return; } }
    try {
      ...
      alert(err.message || 'Add doctor failed');
    }
  });
}
  ...
  if (deleteModalData.callback) {
    try {
      ...
      showDeleteSuccessDialog({
        ...
        subtitle: 'The record has been permanently removed',
        duration: 2000
      });
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
  const patientSearchInput = document.querySelector('#patients .filter-search');
  ...
  // Edit Patient modal close button event listeners
